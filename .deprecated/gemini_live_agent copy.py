...
class GeminiLiveAgent:
    ...
    async def _send_frame_context(self, frame_idx: int) -> None:
        ...
        try:
            ...
            if self.config.is_debug:
                self.console.plog(frame.ground_plane)
        ...
        for fc in tool_call.function_calls:
            ...
            if fc.name == "list_all_detections":
                ...
                if self.current_frame_idx is None:
                    ...
                    if self.config.is_debug:
                        self.console.error(
                            "No current frame set. Please select a frame first."
                        )
                    continue
                ...
                if succ:
                    ...
                    # Use the to_dict() method to get dictionary for internal caching
                    detections_dict = detections.to_dict()
                    
                    # For tool response, convert to array format (Live API requirement)
                    detections_array = list(detections_dict.values())
                    response_payload = {"detections": detections_array}
                    ...
                    # For error cases, return empty detections array and log the error
                    response_payload = {"detections": []}
                    if self.config.is_debug:
                        self.console.error(
                            f"run_aabb_detection failed after {self.config.num_detseg_attempts} attempts: {last_exc}"
                        )
