...
\begin{itemize}
  ...
  \begin{itemize}
    \item \( \oplus \) Supports \textbf{explicit AABBs \& depth queries}, including precise depth at points and object-level (min/max/mean).
    \item \( \oplus \) \textbf{APIs for querying depth}, enabling interaction and dynamic reasoning.
    \item \( \ominus \) Processes \textbf{single RGB-D images} only - no temporal or multi-view support.
    \item \( \ominus \) \textbf{No camera pose awareness} - doesn't model ego-motion.
  \end{itemize}
  \item \textbf{Video-3D LLM}: 3D spatial understanding - depth information \( \rightarrow \) 3D Pos-Encodings.
  \begin{itemize}
    \item \( \oplus \) Uses \textbf{multiple video frames with 3D camera poses} (extrinsics \& intrinsics).
    \item \( \oplus \) Embeds \textbf{position-aware representations} for temporally coherent spatial understanding.
    \item \( \ominus \) \textbf{No explicit depth querying} - no access to raw depth values or masks during inference.
    \item \( \ominus \) No object-centric depth stats or bounding boxes.
  \end{itemize}
\end{itemize}
\end{frame}
...
\begin{frame}{Update Meeting 2 --- Tasks Worked On}
  \begin{itemize}
  \item \textbf{Border detection} based on depth map
  \begin{itemize}
    \item \( \oplus \) Direct use of depth information
    \item \( \oplus \) Depth information makes object detection more accurate and reliable
    \item \( \ominus \) Complicates route calculation.
  \end{itemize}
\end{itemize}
...
\begin{itemize}
  ...
  \begin{itemize}
    \item Depth-enhanced visual understanding (single frame vs multi-frame){\it vs.}
    \item Explicit scene modeling with dynamic scene graphs
    \item End-to-end application using existing models
  \end{itemize}
  \item No labeled RGB-D datasets for navigation.
...
\begin{frame}{Update Meeting 2 --- Plans for Next Week}
  \begin{itemize}
    \item Explore basic Pipeline structure for single frame spatial understanding task.
    \item Implement basic StrayScanner data parser.
    \item Qualitative comparison of Gemini, SpatialBot(, and Video-3D LLM.)
\end{itemize}
\end{frame}
