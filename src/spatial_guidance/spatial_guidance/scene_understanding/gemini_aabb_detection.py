...
from ..utils.gemini_quality_monitor import quality_monitor
...
class GeminiAABBDetSegConfig(BaseConfig["GeminiAABBDetSeg"]):
    ...
    base_system_prompt: str = (
        "You are an expert obstacle detection system for visually impaired navigation. "
        "Detect ONLY critical objects that pose immediate risks:\n\n"
        "PRIORITY 1 - TRIP HAZARDS:\n"
        "- Objects on walking path: curbs, steps, scattered items, cables, construction barriers\n"
        "- Fallen or lying objects: scooters, bikes, debris\n\n"
        "PRIORITY 2 - COLLISION RISKS:\n"
        "- Moving objects: vehicles, bicycles, people\n"
        "- Head-level hazards: low signs, branches, overhangs\n\n"
        "PRIORITY 3 - NAVIGATION AIDS:\n"
        "- Key landmarks: doors, stairs, ramps, handrails\n\n"
        "OUTPUT REQUIREMENTS:\n"
        "- Provide bounding box coordinates [y0, x0, y1, x1] normalized to 0-1000 range\n"
        "- Use clear, specific labels (e.g., 'construction_barrier_orange', 'scooter_fallen_white')\n"
        "- For segmentation masks: generate simple rectangular masks matching the bounding box\n"
        "- Focus on accuracy over completeness - better to miss objects than create false positives\n\n"
        "IGNORE: Sky, distant buildings, decorative elements, walls, general furniture not in walking path"
    )
    ...
    def entrypoint(
        ...
        processed_detections.visualization_rgb = (
            self.visualizer.visualize_rgb_detections(
                input_data.rgb_image, processed_detections
            )
        )
