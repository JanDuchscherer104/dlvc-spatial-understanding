...
from spatial_guidance.pipeline.pipeline import spatial_understanding, PipelineConfig, SpatialUnderstandingPipeline
...
class TestSpatialUnderstandingPipelineFunction(unittest.TestCase):
    ...
    def test_custom_pipeline_execution_flow(self):
        ...
        # Create a barebones pipeline and run sequentially
        ...
        # Since DummyStep returns the input, output should be the PipelineIn instance
        from spatial_guidance.pipeline.pipeline import PipelineIn
        self.assertIsInstance(output, PipelineIn)
        self.assertEqual(output.idx, 0)
        self.assertEqual(output.user_prompt, 'test')
